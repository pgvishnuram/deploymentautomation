...
for deployment in data:
    if "tag" in deployment and deployment["tag"] == docker_tag:
        print(
            f'Key "tag" exists in deployment {deployment["releaseName"]} in the loaded data.'
        )
    else:
        print(deployment)
        ...
        print(
            f"Image Pushed to release {RELEASE_NAME} with URL {DEPLOYMENT_URL} completed"
        )
